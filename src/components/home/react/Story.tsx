...
const StorySection = () => {
  ...
  useEffect(() => {
    const newShowToggleButton = stories.reduce((acc, story) => {
      acc[story.id] =
        story.id !== "card2" && story.review.length > CHARACTER_LIMIT;
      return acc;
  ...
  return (
    <div className="max-w-[1200px] w-full px-4 lg:px-0 md:mx-auto mt-10 md:mt-[67px] mb-10 md:mb-28">
      ...
      <p className="text-center text-base md:text-[22px] text-text mb-10 mt-2 md:mt-4">
        Real experiences from people who found hope and healing at West Valley
        Detox.
      </p>
      ...
      <div className="flex flex-wrap md:flex-nowrap justify-center gap-5 items-start">
        {stories.map((story) => (
          <div
            key={story.id}
            className={`bg-[#fffbeb] cursor-pointer hover:shadow-xl hover:shadow-[#f59e0b] md:min-h-[316px] rounded-lg shadow-[0px_3px_8px_0px_rgba(76,19,7,0.15)] px-5 pt-9 transition-all border-t-4 duration-300 flex flex-col justify-between h-auto w-full ${
              story.borderColor
            } ${story.id === "card2" ? "mb-8 md:mb-0" : ""}`}
          >
            <div onClick={() => toggleCard(story.id)}>
              <div className="flex items-center gap-[10px] mb-5">
                <div className="h-12 w-12 rounded-full bg-[#f59e0b] text-white flex items-center justify-center text-[30px] font-bold">
                  {story.name?.charAt(0)}
                </div>
                <div>
              ...
              <div
                ...
                className={`overflow-hidden transition-all duration-1000 ${
                  expandedCard === story.id || story.id === "card2"
                    ? "max-h-[500px]"
                    : "max-h-[72px]"
                }`}
              >
